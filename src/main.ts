...
import { applyStaticLang, getLang, initLang, onLangChange, setLang, t } from './core/i18n';
...
const MAIL_TO = '417913012@qq.com';
...
function setActive(id: string): void {
  ...
  const hash = def ? `#${def.key}` : '';
  history.replaceState(null, '', hash || location.pathname + location.search);
}

function flash(key: string): void {
  const node = document.querySelector<HTMLElement>(`#${current} [data-key="${key}"]`);
  if (!node) return;
...
async function activate(id: string, openKey?: string, scroll = false): Promise<void> {
  setActive(id);
  const def = MODULES.find((m) => m.id === id);
  if (!def) return;
...
function renderHome(): void {
  ...
  host.querySelectorAll<HTMLElement>('[data-go]').forEach((btn) => {
    btn.addEventListener('click', () => void activate(btn.dataset.go ?? 'm-home', undefined, true));
  });
}
...
function injectJsonLd(): void {
  const data = {
    ...
    '@type': 'WebSite',
    name: '万物经济学 · Economics of Everything',
    url: 'https://MinosIE.github.io/econ-everything/',
    inLanguage: ['zh-CN', 'en'],
    description: t('docDesc'),
    hasPart: MODULES.map((m) => ({
      '@type': 'WebPage',
      name: t(`nav.${m.key}`),
      url: `https://MinosIE.github.io/econ-everything/#${m.key}`,
    })),
  };
...
function deepLink(): void {
  ...
  if (!raw) return;
  // 支持 #concepts 与 #m-concepts?id 两种形态
...
function boot(): void {
  ...
  provideApp({
    ...
    openTarget: (moduleId: string, key: string) => void activate(moduleId, key, true),
